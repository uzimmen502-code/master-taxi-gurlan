...
/// Qarindosh fotosi — `relatives/{userId}/` ostiga siqilgan rasm yuklash.
class RelativePhotoStorage {
  RelativePhotoStorage({FirebaseStorage? storage})
      : _storage = storage ?? FirebaseStorage.instance;

  final FirebaseStorage _storage;
  final _uuid = const Uuid();

  Future<({String url, String path})> uploadPhoto({
    ...
    final path = 'relatives/$userId/$name';
  ...
  Future<Uint8List> _compress(XFile file) async {
    ...
    final outPath = p.join(p.dirname(path), 'rel_${_uuid.v4()}.jpg');
