...
- **TV Market**: `tv_clips_moderation_screen.dart` — АВТО/ҚЎЛДА (`settings/app.tvAutoApprove`), activate/block/delete `tv_clips`. Official social: `TvSocialSettingsBar` + CF `adminGetTvSocialSettings` / `adminSetTvSocialSettings` / `adminPublishTvClipSocial`; trigger `onTvClipSocialPublish` on active. Sidebar badge = pending count.
