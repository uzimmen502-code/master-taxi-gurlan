...
- **Туман ичида** (2026-08 redesign): `yuk_local_drivers/{autoId}` — joylashuv + ish vaqti; **online/heartbeat YO'Q**. Qidiruvchi GPS majburiy (masofa/ETA = searcher GPS ↔ e'lon lat/lng). Ega publish: GPS + work hours; TTL birinchi ever 60 kun / keyingi 10 kun (`yuk_local_owner_meta`). Rank: vehicle prio → readiness → in-radius → ETA → roadKm (**reyting YO'Q**). P0 road/ETA = haversine×1.35 / 27km/h. Faqat qo'ng'iroq. Kartada yulduzcha/live-dot/online menu YO'Q; ⋮ = tahrir/o'chirish. Detal: ish vaqti + muddat. GPS off qidiruvchi: bitta CTA `yuk_local_open_gps_settings` + `yuk_local_need_gps_search` (o'z e'lonlari ko'rinadi). Files: `yuk_local_schedule.dart`, model/repo/ranking/panel/sheet. Demo: `refreshYukDemoPresence` work-hours/TTL backfill. Local tab in-tab search (`CatalogSearch`). IntercityPlaces filters on intercity tab; dark/yellow UI.

## onboarding (`features/onboarding/`)
- Flow 2026-08: **LanguageSelectScreen** = til + tuman (bitta ekran, viloyat/area yashirin; primary `serviceAreaId` avto). **Tuman majburiy:** «Давом этиш» doim bosiladi; tuman yo‘q → keyingi ekranga YO‘Q + sariq banner (`ob_pre_district_required`, `#FEF9C3`); tuman tanlansa banner yo‘qoladi; `_confirm` ichida ham hard gate. Prefs `pre_onboarding_*` + `ServiceConfigHolder.applyGeo` → **OnboardingScreen** (ism/telefon/jins/tug‘ilgan kun + `ob_phone_bind_warning`) → fingerprint login → Home. Zona/GPS/manzil/avto qadami olib tashlangan; ZoneGate + profil zonani saqlaydi.
...
- Phone auth (NOT Firebase SMS): DeviceFingerprintService → `checkDeviceBinding` (europe-west1, trusted жавобда `customToken`) → sign-in (2-чи CF йўқ) → **OnboardingBootstrapScreen** prefs → Home; fallback `createPhoneSession` агар token келмаса. Fingerprint prefetch onboarding ochilganda. profile‖zone‖FCM фон. Warmup both CFs on til screen. **First bind / trusted:** fingerprint auto-link (`fingerprint_first_bind`) + session. **Faza 1 self-serve (2026-08):** `phone_bound_other_device` → conflict sheet → `requestDeviceTransfer` → eski qurilmaga FCM `device_transfer_request` → `respondDeviceTransfer` (faqat fromHash) → link + `users.security.deviceTrustMode=limited` 24s (withdraw gated via `assertNotDeviceTrustLimited`). Collection `device_transfer_requests`. Soft block 30m for wrong-phone attempts. `device_bound_other_phone`/blocked → explainer sheet (no peer transfer). Inbox: `DeviceTransferInboxService` + `listMyPendingDeviceTransfers`. Auto-approve still via `settings/app.deviceBindingAutoApprove`. `needs_verification` → error + retry, no code UI. PhoneReverify/AuthRestore: same fingerprint + `createPhoneSession`. CF `requestPendingCode`/`verifyPendingCodeAndRegister` may still exist server-side but client onboarding does not call them. **Auth claim:** `createPhoneCustomToken` + `setCustomUserClaims({phone_number})`; client `getIdToken(true)` after sign-in.
