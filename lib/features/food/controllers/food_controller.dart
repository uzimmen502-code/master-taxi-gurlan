...
class FoodController extends ChangeNotifier {
  ...
  Future<bool> submitOrder({
    ...
    try {
      ...
      if (uid.length < 9 && cashPaid < cartTotal) {
        isSubmitting = false;
        notifyListeners();
        errorMessage = 'Кошелёк учун телефонни профилда тўлдиринг.';
        notifyListeners();
        return false;
      }

      final walletApplied =
          WalletPayment.maxDebitFromWallet(bal, cartTotal);
      ...
      final cashDue = cartTotal - walletApplied;
      ...
      await BalanceService.placeOrderWithWallet(
        userPhone: userPhone,
        idempotencyKey:
            BalanceService.idempotencyKey('food_place_${userPhone}_$cartTotal'),
        orderBase: orderData,
        decrements: [
