# ava_gurlan (brand display: AVA; package/applicationId unchanged)

Flutter multi-service app (taxi: local + marshrut + intercity; delivery/food/bread) with Firebase backend. Brand display is "AVA" (+ district context); package still ava_gurlan; pubspec `name: ava_gurlan` (renamed from `master_taxi_gurlan` 2026-06). Windows folder `C:\projects\ava_gurlan`, Serena project_name `ava_gurlan`, Android applicationId `uz.ava.gurlan` (all renamed 2026-06). Legacy `com.example.master_taxi_gurlan` kept only as a google-services.json client for migration.
