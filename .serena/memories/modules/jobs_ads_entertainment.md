...
- Search (`cheap_products_screen` / `AdsRepository.searchActiveAds`): client filter on active pool — word match in title/description/`searchTokens` (кирилл↔лотин), min 2 chars; price min/max; sort newest|cheapest|expensive|mostViewed (query+newest → relevance first). Owner update writes `searchTokens`. Backfill: CF `migrateCheapProductTitleLower` also fills empty `searchTokens`.
