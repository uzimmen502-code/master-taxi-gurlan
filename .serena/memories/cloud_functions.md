...
Region default `us-central1`, Node 20 1st Gen. **Auth/binding callables** → `europe-west1` (`authFunctions`): `checkDeviceBinding` (bind-only, no Auth token; `warmup`+`minInstances:1`), **`createPhoneSession`** (createUser+claims+customToken after bind; `warmup`+`minInstances:1`), `registerDeviceBinding`, `requestPendingCode`, `getPendingCodeStatus`, `verifyPendingCodeAndRegister`. Client: `AvaFunctions.auth`; onboarding uses `OnboardingBootstrapScreen` after bind. Helpers from `settlement_ledger.js` imported as `settlementLedger`.
...
- **TV official social** (`functions/tv_social_publish.js` attach): `onTvClipSocialPublish` (tv_clips onWrite → active + socialNetworks → IG Reels / FB Page video / TikTok PULL_FROM_URL). `adminGetTvSocialSettings` / `adminSetTvSocialSettings` / `adminPublishTvClipSocial` (admin|superadmin). Secrets in `settings/tv_social` (CF-only).
- **expireTvContent** (sched, every 15 min) — AVAGram 3-tab content lifecycle: `category=='news' && status=='active' && createdAt < now-48h` → `status='expired'` + `deleteTvClipMedia` (video/poster/`tv_clip_variants/{id}/` deleted from Storage — user's choice, profile stats survive separately in `tv_public_profiles`); `category=='ad' && status=='active' && expiresAt < now` → `status='expired'` only (files kept for possible renewal). Indexes: `tv_clips` (category,status,createdAt) + (category,status,expiresAt) in `firestore.indexes.json`. Feed queries already filter `status=='active'` so expired clips vanish automatically, no rules change needed.
- **publishTvAd** (onCall) — AVAGram «Реклама ва Эълонлар» tariff-based listing: 5 tiers (`TV_AD_TIERS` = basic/visibility/home/premium/pro_max) × 3 durations (`TV_AD_DURATIONS` = 7/15/30 days, no 1-day). Client uploads video/poster to Storage first (existing `tv_publish_screen.dart` flow), then calls this with `idempotencyKey` (`wallet_idempotency/tv_ad_{key}`) + clip fields + `durationDays` + `tier`. Price from `settings/app.tvAdPricing[tier][durationDays]` else `TV_AD_TIER_PRICING_DEFAULT` (rate-card values from product, admin-editable via `tv_ad_pricing_admin_screen.dart` writing `settings/app` directly — no CF needed for price edits, only for the debit+publish). One transaction: debit `users/{uid}.bonusBalance` (only if price>0) + `users/{uid}/wallet_ledger` entry `type:'tv_ad_purchase'` + create `tv_clips/{id}` with `category:'ad'`, `expiresAt`, `adDurationDays`, `adTier`. `tv_clips` rules block client `create`/`update` into `category:'ad'` and block client writes to `expiresAt`/`adDurationDays`/`adTier` — this CF (Admin SDK) is the only path to ad content, mirroring why `bonusBalance` itself is CF-only (`walletFieldsUntouched`). Tier currently only affects price, not actual feed ranking/placement (not built yet).
