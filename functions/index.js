...
exports.onIntercityBookingCancelled = functions.firestore
    ...
    .onUpdate(async (change, context) => {
      ...
      try {
        // O'rin `cancelIntercityBooking` callable ichida qaytariladi
        // (mijozning `seats` ga qo'li yetmaydi). Bu trigger faqat
        // yo'lovchi qulfini tozalaydi — bekor qilish boshqa yo'l bilan
        // (masalan expiry) bo'lganda ham ishlashi uchun.
...
// ═══════════════════════════════════════════════════════════════════════════
// SHAHARARO BRON — o'rin (`seats`) faqat SHU YERDA o'zgaradi
// ═══════════════════════════════════════════════════════════════════════════
//
// Ilgari bron tranzaksiyasi MIJOZDA edi va qoidalar `seats` maydonini
// istalgan chaqiruvchiga ochiq qo'ygan edi (`intercityDriverSeatBookingPatch`):
// har kim istalgan haydovchining bo'sh o'rinlarini qayta yozishi mumkin edi.
// Endi bron/bekor/yakunlash — shu callable'lar orqali, `seats` ga esa
// mijozning umuman qo'li yetmaydi (firestore.rules).

/** Bron hujjatiga tegishli qulflar. */
function intercityLockRefs(driverId, userKey) {
  return {
    driverLock: db.collection('intercity_booking_locks')
      .doc(`${driverId}_${userKey}`),
    passengerLock: db.collection('intercity_passenger_locks').doc(userKey),
  };
}

function intercityNotif(targetPhone, title, body, type, bookingId) {
  return {
    targetPhone: digits(targetPhone) || String(targetPhone || '').trim(),
    title,
    body,
    sent: false,
    type,
...
/** Bir necha telefon formati — 9 va 12 raqamli variantlar. */
function intercityPhoneMatches(a, b) {
  const da = digits(a);
  const dbb = digits(b);
  if (!da || !dbb) return false;
  return canonicalUid(da) === canonicalUid(dbb);
}

/**
 * Ishonchli bron — mijozdagi tranzaksiyaning aynan o'zi, lekin serverda.
 *
 * Yo'lovchi telefoni DATA'dan emas, TOKEN'dan olinadi: shuning uchun
 * boshqa odam nomidan bron qilib bo'lmaydi.
 */
exports.createIntercityBooking = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Login required');
  }
  const userKey = canonicalUid(callerPhone(context));
  if (!userKey || userKey.length < 12) {
    throw new functions.https.HttpsError('unauthenticated', 'Phone token required');
  }
  ...
  if (!Number.isFinite(passengers) || passengers < 1 || passengers > 8) {
    throw new functions.https.HttpsError('invalid-argument', 'passengers 1..8');
  }
  if (!Number.isFinite(pricePerSeat) || pricePerSeat <= 0) {
    throw new functions.https.HttpsError('invalid-argument', 'ride_not_accepting');
  }

  const totalAmount = passengers * pricePerSeat;
  const now = Date.now();
  const expiresAt = admin.firestore.Timestamp.fromMillis(now + 30 * 60 * 1000);
  const departureMs = Number(data.departureTimeMs) || now;

  const bookingRef = db.collection('intercity_bookings').doc();
  const driverRef = db.collection('intercity_drivers').doc(driverId);
  const clientRef = driverRef.collection('clients').doc(userKey);
  const { driverLock, passengerLock } = intercityLockRefs(driverId, userKey);

  const result = await db.runTransaction(async (tx) => {
    const driverSnap = await tx.get(driverRef);
    const clientSnap = await tx.get(clientRef);
    const driverLockSnap = await tx.get(driverLock);
    const passengerLockSnap = await tx.get(passengerLock);

    if (!driverSnap.exists) {
      throw new functions.https.HttpsError('not-found', 'driverNotFound');
    }
    const d = driverSnap.data() || {};

    // Bitta haydovchiga ikkinchi faol bron bo'lmasin.
    for (const lock of [driverLockSnap, passengerLockSnap]) {
      const activeId = lock.exists ? String((lock.data() || {}).bookingId || '') : '';
      if (!activeId) continue;
      const existing = await tx.get(db.collection('intercity_bookings').doc(activeId));
      if (!existing.exists) continue;
      const st = String((existing.data() || {}).status || '');
      if (st === 'pending' || st === 'confirmed') {
        throw new functions.https.HttpsError(
          'failed-precondition',
          lock === driverLockSnap ? 'alreadyBooked' : 'alreadyActive',
        );
      }
    }

    if (d.isActive === false) {
      throw new functions.https.HttpsError('failed-precondition', 'driverInactive');
    }
    const seats = Number(d.seats) || 0;
    if (seats < passengers) {
      throw new functions.https.HttpsError('failed-precondition', 'notEnoughSeats');
    }

    const autoAccept = d.autoAcceptBookings === true;
    ...
    tx.set(bookingRef, {
      ...
      userGender: String(data.userGender || ''),
      userBirthDate: String(data.userBirthDate || ''),
      driverId,
      driverPhone: String(d.phone || data.driverPhone || ''),
      driverName: String(d.name || data.driverName || ''),
      carNumber: String(d.plate || data.carNumber || ''),
      ...
      totalAmount,
      status,
      driverRouteLabel: String(data.driverRouteLabel || ''),
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      expiresAt,
      departureTime: admin.firestore.Timestamp.fromMillis(departureMs),
      ...(status === 'confirmed'
      ...
      ...(data.reportStamp && typeof data.reportStamp === 'object'
        ? data.reportStamp
        : {}),
    });
    ...
    if (!clientSnap.exists) {
      clientPatch.firstBookingAt = admin.firestore.FieldValue.serverTimestamp();
      clientPatch.completedCount = 0;
    }
    tx.set(clientRef, clientPatch, { merge: true });

    const driverPhone = String(d.phone || '');
    if (driverPhone) {
      tx.set(db.collection('notifications').doc(), intercityNotif(
        driverPhone,
        status === 'pending' ? '🔔 Янги брон сўрови!' : '🚗 Янги бронь!',
        String(data.driverAlertBody || 'Янги брон'),
        status === 'pending' ? 'intercity_booking_pending' : 'intercity_booking',
        bookingRef.id,
      ));
    }
    if (status === 'confirmed') {
      tx.set(db.collection('notifications').doc(), intercityNotif(
        userKey,
        String(data.confirmedTitle || '✅ Брон тасдиқланди'),
        String(data.pickupBody || ''),
...
/** Bekor qilish — yo'lovchi, haydovchi yoki admin; o'rinlar qaytariladi. */
async function cancelIntercityBookingInternal(bookingId, reason, callerKey, isAdminCaller) {
  ...
  if (!isAdminCaller
      && !intercityPhoneMatches(b.userPhone, callerKey)
      && !intercityPhoneMatches(b.driverId, callerKey)
      && !intercityPhoneMatches(b.driverPhone, callerKey)) {
    throw new functions.https.HttpsError('permission-denied', 'Not your booking');
  }

  const driverId = String(b.driverId || '');
  const userKey = canonicalUid(b.userPhone);
  const driverRef = db.collection('intercity_drivers').doc(driverId);
  const clientRef = userKey ? driverRef.collection('clients').doc(userKey) : null;
  const { driverLock } = intercityLockRefs(driverId, userKey || 'x');
  ...
  const cancelled = await db.runTransaction(async (tx) => {
    ...
    const lockSnap = userKey ? await tx.get(driverLock) : null;

    const passengers = Number(fresh.passengers) || 0;
    if (driverSnap.exists && passengers > 0) {
      const d = driverSnap.data() || {};
      const capacity = Number(d.seatCapacity);
      const restored = (Number(d.seats) || 0) + passengers;
      tx.update(driverRef, {
        seats: Number.isFinite(capacity) && capacity > 0
          ? Math.min(restored, capacity)
          : restored,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      });
    }
    ...
    if (clientRef && clientSnap && clientSnap.exists) {
      tx.update(clientRef, {
        ...
        totalSpent: admin.firestore.FieldValue.increment(
          -(Number(fresh.totalAmount) || 0)),
        lastBookingAt: admin.firestore.FieldValue.serverTimestamp(),
      });
    }

    if (lockSnap && lockSnap.exists) {
      tx.update(driverLock, {
...
exports.cancelIntercityBooking = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Login required');
  }
  const callerKey = canonicalUid(callerPhone(context));
  if (!callerKey || callerKey.length < 12) {
    throw new functions.https.HttpsError('unauthenticated', 'Phone token required');
  }
  ...
  // (a) bitta bron
  if (bookingId) {
    const cancelled = await cancelIntercityBookingInternal(
      bookingId, reason, callerKey, false);
    if (cancelled && cancelled.userPhone) {
      await db.collection('notifications').add(intercityNotif(
        cancelled.userPhone,
        String(data.notifyTitle || '❌ Брон бекор қилинди'),
        String(data.notifyBody || ''),
        'intercity_booking_cancelled',
        bookingId,
      ));
    }
    return { ok: true, cancelled: cancelled ? 1 : 0 };
  }

  // (b) haydovchining barcha faol bronlari (reys yopilganda/yangi reys)
  if (!driverId) {
    throw new functions.https.HttpsError('invalid-argument', 'bookingId yoki driverId');
  }
  if (!intercityPhoneMatches(driverId, callerKey)) {
    throw new functions.https.HttpsError('permission-denied', 'Not your rides');
  }
  const snap = await db.collection('intercity_bookings')
    .where('driverId', '==', driverId)
    .where('status', 'in', ['pending', 'confirmed'])
  ...
  for (const doc of snap.docs) {
    const cancelled = await cancelIntercityBookingInternal(
      doc.id, reason, callerKey, true);
    if (!cancelled) continue;
    count++;
    if (cancelled.userPhone) {
      await db.collection('notifications').add(intercityNotif(
        cancelled.userPhone,
        String(data.notifyTitle || '❌ Рейс бекор — бронингиз ҳам ёпилди'),
        String(data.notifyBody || ''),
        'intercity_booking_cancelled',
        doc.id,
      ));
...
/** Haydovchi safarni yakunlaydi — o'rin qaytariladi. */
exports.completeIntercityBooking = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'Login required');
  }
  const callerKey = canonicalUid(callerPhone(context));
  ...
  const bookingRef = db.collection('intercity_bookings').doc(bookingId);
  const pre = await bookingRef.get();
  if (!pre.exists) return { ok: true, completed: 0 };
  const b = pre.data() || {};
  if (!intercityPhoneMatches(b.driverId, callerKey)
      && !intercityPhoneMatches(b.driverPhone, callerKey)) {
    throw new functions.https.HttpsError('permission-denied', 'Not your booking');
  }
  const st = String(b.status || '');
  if (st !== 'pending' && st !== 'confirmed') return { ok: true, completed: 0 };
  ...
  await db.runTransaction(async (tx) => {
    ...
    // Reys hali e'londa bo'lsa — o'rin qaytadi (mijozdagi mantiqning o'zi).
    if (driverSnap.exists) {
      const d = driverSnap.data() || {};
      if (d.isActive !== false) {
        const capacity = Number(d.seatCapacity);
        const restored = (Number(d.seats) || 0) + (Number(fresh.passengers) || 0);
        tx.update(driverRef, {
          seats: Number.isFinite(capacity) && capacity > 0
            ? Math.min(restored, capacity)
            : restored,
  ...
  if (b.userPhone) {
    await db.collection('notifications').add(intercityNotif(
      b.userPhone,
      String(data.notifyTitle || '⭐ Сафар якунланди'),
      String(data.notifyBody || ''),
      'intercity_trip_completed',
      bookingId,
    ));
  }
  return { ok: true, completed: 1 };
});
