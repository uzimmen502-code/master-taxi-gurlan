...
Colors: `jobs_colors.dart` — scaffold #F7FFEF, bar/CTA #4E9F00, service #0277BD, urgent #C62828, ink/muted/hint hierarchy.
...
- Flow: create via CF `submitJobAd` → `pending` unless `jobsAutoApprove` then `active`. Add sheet: scroll + sticky submit (keyboard-safe); awaits profile phone before submit; success snackbar reflects `active` vs pending. Daily limit counts only jobs types (`work|service|ad`), not `cheap_product`. Admin: ҚЎЛДА/АВТО + `adminSetJobsAutoApprove`; complaints via CF `submitJobComplaint` (auth, daily 20, no own-ad, open-dupe). Client create on `ads`/`complaints` blocked. Owner edit/delete via `AdCard` «Таҳрирлаш» → `showEditAdSheet` + `updateAdByOwner` (rules `adOwnerPatchOk`); gated by `phonesMatch`. Feed limits 300/500; `watchAdsByAuthor` uses `phoneAliases` whereIn. Expire: `expirePendingTrips` — jobs active|pending → completed; cheap_product → inactive.
