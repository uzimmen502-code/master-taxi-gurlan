# Tech Stack
- Flutter / Dart (mobile + web). pubspec `name: ava_gurlan` (renamed from `master_taxi_gurlan` 2026-06; lib uses relative imports, only 3 test files used `package:` imports). Android applicationId `uz.ava.gurlan`. Windows folder & Serena project_name still `master_taxi_gurlan`.
